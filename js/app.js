    ...
    grid.innerHTML = items.map((it) => `
      <div class="item-card" data-id="${it.id}">
        ...
        <div class="item-meta">
          ...
          <div class="item-sub">${esc([it.category, it.subCategory].filter(Boolean).join(' · '))}${it.color ? (' · ' + esc(it.color)) : ''}</div>
...
async function openBatchImportModal() {
  ...
  $('#batchInput').addEventListener('change', async (e) => {
    ...
    for (const f of files) {
      try { const d = await compressImage(f); state._batch.push({ file: f, dataUrl: d, category: def, subCategory: defSub }); } catch (_) {}
    }
    renderGrid();
  });
  $('#batchApplyAll').addEventListener('click', () => {
    const def = $('#batchCat').value;
    const defSub = $('#batchSubCat').value;
    state._batch.forEach((b) => { b.category = def; b.subCategory = defSub; });
    renderGrid();
    toast('已应用默认分类');
  });
